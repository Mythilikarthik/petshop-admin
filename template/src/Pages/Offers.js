...
// import { useLocation, useNavigate, useSearchParams } from 'react-router-dom'; // Imported hooks for reading/managing the query state
// import { 
...
// console.log("Verified Offer ID:", targetOfferId);
...
import { 
  ...
  BiMap, BiTimeFive, BiShow, BiSolidHeart,
  BiPhone, BiMessageRoundedDots, BiChevronLeft, BiChevronRight,
  BiShareAlt // 🔹 Added Share Icon import
} from 'react-icons/bi';
import { useAuthProtectedAction } from '../hooks/useAuthProtectedAction';
...
const Offers = () => {
  ...
  // 🔹 Protected wrapper logic dedicated for share interactions
  ...
  // 🔹 Native Dynamic Sharing Execution Engine Method
  const handleShareClick = async (post) => {
    ...
    if (navigator.share) {
      try {
        ...
        console.log('User cancelled native system share sheet interaction context.', err);
      }
    } else {
      try {
        await navigator.clipboard.writeText(fullyConstructedUrl);
        alert('Offer link copied successfully to clipboard area details!');
        await handleActionClick(post._id, 'share');
      } catch (err) {
        console.error('Failed to register copied selection elements:', err);
      }
    }
  };
                              ...
                              <Card className="h-100 border-0 shadow-sm overflow-hidden" style={{ borderRadius: '16px' }}>
                                ...
                                <Card.Body className="d-flex flex-column justify-content-between p-4">
                                  ...
                                  <ButtonGroup className="w-100 gap-2">
                                    ...
                                    {post.primaryActions?.includes('book_now') && (
                                      <Button 
                                        onClick={() => {
                                          ...
                                          // Ensure the URL exists before trying to open it
                                          if (post.bookNowUrl) {
                                            // Safe check to prepend https:// if the user forgot it, preventing broken local routing
                                    ...
                                    {/* 🔹 Always Accessible Action Share Trigger Button */}
                                    <Button 
                                      onClick={() => handleShareProtected(post)}
                                      variant="outline-primary"
                                      className="d-flex align-items-center justify-content-center border custom-share-btn"
                                      // style={{ borderRadius: '10px', width: '55px', padding: '0.6rem', borderColor: '#cbd5e1', color: '#475569' }}
                                    >
                                      <BiShareAlt size={18} />
                                    </Button>
                                  </ButtonGroup>
