...
const Offers = () => {
  ...
  // Fetch feeds without automatically triggering views on page load
  useEffect(() => {
    const fetchLiveFeeds = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/offers`);
  ...
  // Handle card click to track a view once per session
  const handleCardClick = async (postId) => {
    const viewedOffers = JSON.parse(localStorage.getItem('viewed_offers')) || [];
    
    if (!viewedOffers.includes(postId)) {
      // Update local state instantly for UI responsiveness
      setPosts(prev => prev.map(p => {
        if (p._id === postId) {
          const currentViews = p.analytics?.viewedByIPs?.length || p.analytics?.views || 0;
          return {
            ...p,
            analytics: { 
              ...p.analytics, 
              views: currentViews + 1,
              viewedByIPs: p.analytics?.viewedByIPs ? [...p.analytics.viewedByIPs, 'local'] : ['local']
            }
          };
        }
        return p;
      ...
      // Store in localStorage so it doesn't count again for this browser session
      localStorage.setItem('viewed_offers', JSON.stringify([...viewedOffers, postId]));

      // Send update to backend API
      try {
        await fetch(`${API_BASE}/api/offers/${postId}/track-view`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' }
        });
      } catch (err) {
  ...
  // Auto-slide effect for posts containing multiple media items
  useEffect(() => {
    const interval = setInterval(() => {
      setCarouselIndices(prevIndices => {
        const updatedIndices = { ...prevIndices };
                    ...
                    return (
                      <Col xs={12} md={6} key={post._id}>
                        ...
                        >
                          ...
                          <div 
                            className="position-relative bg-dark style-media-container" 
