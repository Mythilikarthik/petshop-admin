...
import excelFile from '../assets/pet-boarding-chennai.xlsx';
...
export default function PetGroomingCityPage() {
  ...
  const renderFAQContent = (content, h1Text) => {
...
const formatCityName = (str) => 
  ...
  // Generate the dynamic H2 title
  const h2Title =`Frequently Asked Questions About Pet Boarding in ${formatCityName(cityName)}` ;
  const blocks = raw.split(/\r?\n+/).filter(Boolean);
  let htmlOutput = `<h2>${h2Title}</h2>`;
  ...
  blocks.forEach((block) => {
    ...
    if (block.includes('|')) {
      ...
      question = parts[0].replace(/^(Q:|Q\.)\s*/i, '');
      answer = parts[1] ? parts[1].replace(/^(A:|A\.)\s*/i, '') : '';
    } else if (block.toLowerCase().startsWith('q:') || block.toLowerCase().startsWith('q.')) {
      question = block.replace(/^(Q:|Q\.)\s*/i, '');
    } else if (block.toLowerCase().startsWith('a:') || block.toLowerCase().startsWith('a.')) {
      answer = block.replace(/^(A:|A\.)\s*/i, '');
    } else {
      question = block;
    }
    ...
    if (question) {
      htmlOutput += `
        <div style="margin-bottom: 1.5rem;">
          <div style="background: #fff5f0; padding: 12px 16px; border-radius: 8px; border-left: 4px solid #ff4e00; font-weight: 700; color: #111827; margin-bottom: 0.75rem;">
            ${question}
          </div>
          ${answer ? `<div style="padding-left: 0.5rem; color: #4b5563; line-height: 1.7; text-align: justify;">${answer}</div>` : ''}
        </div>
      `;
    } else if (answer) {
      htmlOutput += `
        <div style="padding-left: 0.5rem; color: #4b5563; line-height: 1.7; margin-bottom: 1.5rem; text-align: justify;">
          ${answer}
        </div>
      `;
    }
  ...
  const formatDirectoryListings = (htmlContent, h1Text) => {
  if (!htmlContent) return '';
  
  const parser = new DOMParser();
  const doc = parser.parseFromString(htmlContent, 'text/html');
  let h2Tag = doc.querySelector('h2');
  
  // Check if h2Tag exists (NOT null)
  if (h2Tag) {
    h2Tag.textContent = h1Text ? `${h1Text}` : h2Tag.textContent;
  } else {
    // If it doesn't exist, create it dynamically
    h2Tag = doc.createElement('h2');
    h2Tag.textContent = h1Text ? `${h1Text}`  : 'Verified Pet Boarding Facilities';
    doc.body.insertBefore(h2Tag, doc.body.firstChild);
  }
...
const renderFormattedContent = (content) => {
        ...
        <Row className="mb-3">
          <Col lg={12}>
            <Breadcrumb className="custom-breadcrumb">
              ...
              <Breadcrumb.Item linkAs={Link} linkProps={{ to: '/pet-boarding-chennai' }}>
                Pet Boarding Chennai
              </Breadcrumb.Item>
  ...
  <Col md={6} className="d-flex align-items-stretch">
    <Card className="custom-card w-100 p-4">
      <Card.Body 
        className="excel-content p-0 text-justify"
        dangerouslySetInnerHTML={{ __html: formatDirectoryListings(currentCityData['Services Offered'], currentCityData.H1) }}
      />
          ...
          {currentCityData['Average Pet Boarding Prices'] && (
            <Col md={6} className="d-flex align-items-stretch">
              <Card className="custom-card w-100 p-4">
                <Card.Body 
                  className="excel-content p-0"
                  dangerouslySetInnerHTML={renderFormattedContent(currentCityData['Average Pet Boarding Prices']) }
                />
              </Card>
            </Col>
          )}
          ...
          {currentCityData['Local Area Pet Boarding Guide'] && (
            <Col md={6} className="d-flex align-items-stretch">
              <Card className="custom-card p-4">
                <Card.Body 
                  className="excel-content p-0 text-justify"
                  dangerouslySetInnerHTML={{ __html: formatDirectoryListings(currentCityData['Local Area Pet Boarding Guide'], currentCityData.H1) }}
                />
              </Card>
            </Col>
          )}
          {currentCityData['After Boarding'] && (
            <Col md={12} className="d-flex align-items-stretch">
              <Card className="custom-card p-4">
                <Card.Body 
                  className="excel-content p-0 text-justify"
                  dangerouslySetInnerHTML={{ __html: formatDirectoryListings(currentCityData['After Boarding'], currentCityData.H1) }}
                />
              </Card>
            </Col>
          )}
  ...
  <Col md={12}>
    <Card className="custom-card p-4">
      <Card.Body 
        ...
        dangerouslySetInnerHTML={renderFAQContent(currentCityData.FAQ, currentCityData.H1)}
      />
    </Card>
  </Col>
)}
