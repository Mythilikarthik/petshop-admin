...
export default function PetShops() {
  ...
  const area = areaName.split("-chennai");
console.log(area);
  useEffect(() => {
    fetch(excelFile)
  ...
  const currentAreaData = data.find(
    (row) => createSlug(row.Areas) === createSlug(area[0])
  );
  ...
  border-left: 3px solid #ff4e00;
      ...
      <Container>
        ...
        <Row className="mb-3">
          <Col lg={12}>
            <Breadcrumb className="custom-breadcrumb">
              ...
              <Breadcrumb.Item linkAs={Link} linkProps={{ to: '/pet-shops' }}>
                Pet Shop
              </Breadcrumb.Item>
              <Breadcrumb.Item active>
                {currentAreaData.Area || areaName}
              </Breadcrumb.Item>
            </Breadcrumb>
          </Col>
        </Row>
        ...
        <Row className="g-4">
          ...
          {currentAreaData['pet_shop_listings'] && (
            <Col md={12} className="d-flex align-items-stretch">
              <Card className="custom-card w-100 p-4">
                <Card.Body 
                    className="excel-content p-0"
                    dangerouslySetInnerHTML={renderFormattedContent(currentAreaData['pet_shop_listings'])}
                    />
              </Card>
            </Col>
          )}
          ...
          {currentAreaData['Pet_Shops_Near'] && (
  <Col md={6} className="d-flex align-items-stretch">
    <Card className="custom-card w-100 p-4">
      <Card.Body 
        className="excel-content p-0"
        dangerouslySetInnerHTML={renderFormattedContent(currentAreaData['Pet_Shops_Near'])}
      />
    </Card>
  </Col>
)}

          {currentAreaData['Pet_shop_services'] && (
            <Col md={6} className="d-flex align-items-stretch">
              <Card className="custom-card w-100 p-4">
                <Card.Body 
                  className="excel-content p-0"
                  dangerouslySetInnerHTML={{ __html: currentAreaData['Pet_shop_services'] }}
                />
              </Card>
            </Col>
          )}

          {currentAreaData['Pet_Supplies_&_Pricing'] && (
            <Col md={12} className="d-flex align-items-stretch">
              <Card className="custom-card w-100 p-4">
                <Card.Body 
                  className="excel-content p-0"
                  dangerouslySetInnerHTML={renderFormattedContent(currentAreaData['Pet_Supplies_&_Pricing'])}
                />
              </Card>
            </Col>
          )}

          {currentAreaData['Pet_Friendly_Areas_Near'] && (
            <Col md={12} className="d-flex align-items-stretch">
              <Card className="custom-card p-4">
                <Card.Body 
                  className="excel-content p-0"
                  dangerouslySetInnerHTML={renderFormattedContent(currentAreaData['Pet_Friendly_Areas_Near'])}
                />
              </Card>
            </Col>
          )}
