...
export default function PetBoardingDirectoryPage() {
    ...
    fetch(excelFile)
      ...
      .then((buffer) => {
        ...
        const cityList = parsedRows
          ...
          .map((row) => ({
            name: row.City,
            slug: String(row.City).toLowerCase().trim().replace(/\s+/g, '-'),
      ...
      <Container>
        ...
        <Row className="justify-content-center text-center mb-5">
          <Col lg={8}>
            ...
            >
              Pet Boarding Directory
            </span>
            <h1 className="fw-bold text-dark mb-3">Find Pet Boarding Near You</h1>
          ...
          {filteredCities.length > 0 ? (
            filteredCities.map((city) => (
              <Col key={city.slug} sm={6} md={4} lg={3}>
                <Link to={`/pet-boarding-chennai/${city.slug}`} className="text-decoration-none">
